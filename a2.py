...
#         textSplit = [word.lower() for word in textSplit]
        for i in textSplit:
            i = i.strip()
            if i not in dic:
                if len(i) < 3 or i in stopwords or i in stopwords2:
                    continue
                dic[i] = 0
...
def extract_features(samples):
    ...
    dicofwords, wordcounts  = read_text(samples)
    wordstoremove = {}
    for k in sorted(wordcounts, key=wordcounts.get, reverse=False):
        if  wordcounts[k] <= 50:
            wordstoremove[k] = wordcounts[k]
    updatedDict = []
    for i in range(len(dicofwords)):
        d = {}
        d = dicofwords[i].copy()
        for k, v in dicofwords[i].items():
            if k in wordstoremove:
                d.pop(k)
        updatedDict.append(d)
    df = pd.DataFrame(updatedDict)
    res = df.to_numpy()
...
def get_classifier(clf_id):
    if clf_id == 1:
        clf  = LogisticRegression() # <--- REPLACE THIS WITH A SKLEARN MODEL
    elif clf_id == 2:
...
def main(model_id=None, n_dim=False):
    ...
    #part 2
    if n_dim:
        # n_dim = int(0.50 * X.shape[1])
        # print("Original dimensions", X.shape[1])
        # print("50 percent of the Original features", n_dim)
        print("\n------------PART 2-----------")
        X = part2(X, n_dim)
